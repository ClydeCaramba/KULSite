...
class cbCalendars
{
	...
	public function cbAddCalendar( $name, $label = null, $required = false, $value = null, $readOnly = false, $showTime = false, $minYear = null, $maxYear = null, $attributes = null, $serverTimeOffset = true, $offsetOverride = null )
	{
		...
		if ( $readOnly ) {
			...
			$return							=	'<input type="hidden" name="' . htmlspecialchars( $name ) . '" id="' . htmlspecialchars( $inputId ) . '" value="' . htmlspecialchars( $value ) . '" class="cbDatePicker"' . ( trim( $attributes ) ? ' ' . $attributes : null ) . ' />'
