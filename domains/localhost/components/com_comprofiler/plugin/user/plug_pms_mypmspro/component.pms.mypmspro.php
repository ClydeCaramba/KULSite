...
class CBplug_pmsmypmspro extends cbPluginHandler
{
	...
	public function showMessages( $user, $type = null )
	{
		...
		$pageNav->setBaseURL( $_CB_framework->pluginClassUrl( $this->element, false, array( 'action' => 'messages', 'pmsearch' => ( $searching ? $search : null ) ) ) );
	...
	public function showMessage( $id, $user )
	{
		...
		if ( ! $row->get( 'id', 0, GetterInterface::INT ) ) {
			PMSHelper::returnRedirect( $returnUrl, CBTxt::T( 'Message does not exist.' ), 'error' );
				   ...
				   && ( $user->get( 'id', 0, GetterInterface::INT ) != 0 ) ) ) {
		...
		if ( $messageEditor == 2 ) {
			$input['message']			=	cbTooltip( null, CBTxt::T( 'Input your reply.' ), null, null, null, $_CB_framework->displayCmsEditor( 'message', $this->input( 'post/message', null, GetterInterface::HTML ), 350, 150, 35, 4 ), null, 'class="d-block clearfix"' );
		} else {
			$messageTooltip				=	cbTooltip( null, CBTxt::T( 'Input your reply.' ), null, null, null, null, null, 'data-hascbtooltip="true"' );

			$input['message']			=	'<textarea id="message" name="message" class="form-control" cols="35" rows="4"' . $messageTooltip . ( $messageLimit ? cbValidator::getRuleHtmlAttributes( 'maxlength', $messageLimit ) : null ) . '>' . htmlspecialchars( $this->input( 'post/message', null, GetterInterface::STRING ) ) . '</textarea>';
		}

		$input['message_limit']			=	null;

		if ( $messageLimit ) {
			$js							=	"$( '.pmMessageEdit textarea' ).on( 'change keyup', function() {"
	...
	public function showMessageEdit( $id, $user )
	{
		...
		if ( ! $row->get( 'id', 0, GetterInterface::INT ) ) {
			if ( ( ! $user->get( 'id', 0, GetterInterface::INT ) ) && ( ! $this->params->get( 'messages_public', false, GetterInterface::BOOLEAN ) ) ) {
		...
		if ( ! $row->get( 'id', 0, GetterInterface::INT ) ) {
			...
			if ( $to && is_numeric( $to ) ) {
				$to						=	CBuser::getUserDataInstance( (int) $to )->get( 'username', null, GetterInterface::STRING );
			}
		...
		if ( $messageEditor == 2 ) {
			$input['message']			=	cbTooltip( null, CBTxt::T( 'Input your private message.' ), null, null, null, $_CB_framework->displayCmsEditor( 'message', $this->input( 'post/message', $row->get( 'message', null, GetterInterface::HTML ), GetterInterface::HTML ), 350, 175, 35, 4 ), null, 'class="d-block clearfix"' );
		} else {
			$messageTooltip				=	cbTooltip( null, CBTxt::T( 'Input your private message.' ), null, null, null, null, null, 'data-hascbtooltip="true"' );

			$input['message']			=	'<textarea id="message" name="message" class="form-control" cols="35" rows="4"' . $messageTooltip . ( $messageLimit ? cbValidator::getRuleHtmlAttributes( 'maxlength', $messageLimit ) : null ) . '>' . htmlspecialchars( $this->input( 'post/message', $row->get( 'message', null, GetterInterface::STRING ), GetterInterface::STRING ) ) . '</textarea>';
		}
	...
	private function saveMessageEdit( $id, $user )
	{
		...
		if ( $this->params->get( 'messages_editor', 2, GetterInterface::INT ) == 2 ) {
		...
		if ( ! $row->get( 'id', 0, GetterInterface::INT ) ) {
			...
			if ( $reply ) {
				$canReply				=	$this->params->get( 'messages_reply', 2, GetterInterface::INT );

				$replyTo->load( (int) $reply );

				if ( ! $replyTo->get( 'id', 0, GetterInterface::INT ) ) {
					PMSHelper::returnRedirect( $returnUrl, CBTxt::T( 'Message does not exist.' ), 'error' );
				} elseif ( ( ( ! $canReply ) || ( ( $canReply == 2 ) && ( ! $replyTo->get( 'from_user', 0, GetterInterface::INT ) ) ) )
			...
			if ( ! $toArray ) {
				$_CB_framework->enqueueMessage( CBTxt::T( 'To user not specified.' ), 'error' );
			...
			foreach ( $toArray as $k => $to ) {
				...
				if ( $global ) {
					...
					if ( ! $toUser->get( 'id', 0, GetterInterface::INT ) ) {
						$_CB_framework->enqueueMessage( CBTxt::T( 'To user does not exist.' ), 'error' );
				...
				if ( $reply && ( ! $replyTo->get( 'from_user', 0, GetterInterface::INT ) ) ) {
					...
					$replyToName		=	getNameFormat( $user->get( 'name', null, GetterInterface::STRING ), $user->get( 'username', null, GetterInterface::STRING ), Application::Config()->get( 'name_format', 3, GetterInterface::INT ) );
					...
					if ( ! $cbNotification->sendFromSystem( $toUser, $subject, $message, false, 1, null, null, null, array(), true, $this->params->get( 'messages_notify_from_name', null, GetterInterface::STRING ), $this->params->get( 'messages_notify_from_email', null, GetterInterface::STRING ), $replyToName, $replyToEmail ) ) {
	...
	private function deleteMessage( $id, $user )
	{
		...
		if ( ( ! $row->get( 'from_user', 0, GetterInterface::INT ) ) || ( ! $row->get( 'to_user', 0, GetterInterface::INT ) ) ) {
			...
			if ( $row->get( 'to_user_delete', 0, GetterInterface::INT ) || ( ! $row->getRead() ) ) {
				$delete	=	true;
			} else {
				$row->set( 'from_user_delete', 1 );
			}
		} elseif ( $user->get( 'id', 0, GetterInterface::INT ) == $row->get( 'to_user', 0, GetterInterface::INT ) ) {
			if ( $row->get( 'from_user_delete', 0, GetterInterface::INT ) ) {
				$delete	=	true;
			} else {
