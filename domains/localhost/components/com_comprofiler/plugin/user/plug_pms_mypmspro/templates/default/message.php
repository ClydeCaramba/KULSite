...
if ( ( $row->get( 'from_user', 0, GetterInterface::INT ) == $user->get( 'user_id', 0, GetterInterface::INT ) ) || ( $row->get( 'to_user', 0, GetterInterface::INT ) == $user->get( 'user_id', 0, GetterInterface::INT ) ) || Application::MyUser()->isGlobalModerator() ) {
	$menu[]				=	'<li class="pmMessageMenuItem" role="presentation"><a href="javascript: void(0);" onclick="cbjQuery.cbconfirm( \'' . addslashes( CBTxt::T( 'Are you sure you want to delete this message?' ) ) . '\' ).done( function() { window.location.href = \'' . addslashes( $_CB_framework->pluginClassUrl( $this->element, true, array( 'action' => 'message', 'func' => 'delete', 'id' => $row->get( 'id', 0, GetterInterface::INT ), cbSpoofField() => cbSpoofString( null, 'plugin' ), 'return' => PMSHelper::getReturn() ) ) ) . '\'; })" class="dropdown-item" role="menuitem"><span class="fa fa-trash-o"></span> ' . CBTxt::T( 'Delete' ) . '</a></li>';
}
...
<div class="pmMessage pmMessageDefault">
	...
	if ( ( ( $canReply == 1 ) || ( ( $canReply == 2 ) && $row->get( 'from_user', 0, GetterInterface::INT ) ) )
