	...
	static function showBlogTab( $rows, $pageNav, $searching, $input, $viewer, $user, /** @noinspection PhpUnusedParameterInspection */ $model, $tab, $plugin )
	{
		...
		if ( $rows ) foreach ( $rows as $row ) {
			...
			if ( $cbModerator || $profileOwner || $canPublish ) {
				...
				$return				.=								cbTooltip( 1, $menuItems, null, 'auto', null, '<span class="ml-2 fa fa-ellipsis-v"></span>', null, 'class="pull-right cbDropdownMenu blogsMenu" data-cbtooltip-menu="true" data-cbtooltip-classes="qtip-nostyle" data-cbtooltip-open-classes="active"' );
			}
